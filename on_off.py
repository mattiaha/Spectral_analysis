...
irfs["bkg"].plot_at_energy(
    ["50 GeV", "100 GeV", "1 TeV", "5 TeV", "10 TeV", "20 TeV"]
)
plt.show()
...
log_lb = dataset_stacked.stat_sum()


print(dm_model)
stacked_fit = Fit()
result_stacked = stacked_fit.run([dataset_stacked])
scale_hat = result_stacked.parameters["scale"].value
print(result_stacked)

log_lsb = result_stacked.total_stat

q = log_lb-log_lsb
if scale_hat < 0:
    q *= -1
print("Test statistic is " + str(q))
print(result_stacked)
...
for j in range(n_steps):
    scale = j/(n_steps-1.0)*2.0
    dm_model.parameters["scale"].value = scale
    dataset_stacked.models = dm_model
    l = dataset_stacked.stat_sum()
    log_l.append(l)
    scales.append(scale)
